...
- **实验验证**: 静止启动守卫的有效性验证由 [实验手册 §7.11](实验手册.md#711-静止启动守卫消融-r2-推荐) 覆盖
  (同地点 2 段 bag, 启动时立即遥控 vs 静止 1 s, 闭环漂移对比)。

### 7.6 退化判据的可观测性与阈值鲁棒性

§4.1 的判据 `feat_num < 200 ∨ res_mean > 0.15` 只是**首次实机验证初始值**, 需要两类实验
证据才能进入论文:

- **触发空间分布** (验证"在该激活的位置激活"): 退化触发率 30% 可能集中在长廊, 也可能
  全程随机抖动. 由 [`laserMapping.cpp` 中的 `[OdomFrame]` 逐帧日志] 配合
  [`evaluate_slam.sh` 阶段 6](../scripts/evaluate_slam.sh) 生成 `degradation_spatial_*.png`,
  期望红点聚集在退化走廊段. 详见 [实验手册 §7.9](实验手册.md#79-退化触发空间分布-n1-必做)。
- **阈值敏感性 3×3 表** (验证"不是凑出来的"): `feat ∈ {100,200,400} × res ∈ {0.10,0.15,0.25}`,
  期望触发率 20%-60% 工作点附近闭环漂移变化 < 30%. 由
  [`threshold_sensitivity` 聚合脚本](../go1_bringup/threshold_sensitivity.py) 生成 markdown 表.
  详见 [实验手册 §7.10](实验手册.md#710-退化阈值敏感性扫描-n2-必做)。

这两项是论文里把创新点从"我做了"升级到"我证明了它在哪儿生效、对参数不敏感"的关键证据。
